...
var configHelpers = {
  ...
  isConfigChanged: function () {
    return true;
    var fileConfig = fs.readFileSync(configPath, 'utf-8')
    var configString = JSON.stringify(config, null, 2);
    return fileConfig !== configString;
  },
  ...
  */
  ...
  init: function (customHome, options) {
    ...
    var onConfigChanged = function () {
      ...
      if (configHelpers.isConfigChanged()) {
...
var nodeHelpers = {
  ...
  /**
   * Clear from the require cache, the given app.
   *
...
// TODO write comment, put it in a module.
var restartWatcher = {
  watchers: [],
  trigger: function () {
  ...
  off: function (cb) {
    var index = restartWatcher.watchers.indexOf(cb);
    restartWatcher.watchers.splice(index, 1);
  },
  one: function (cb) {
    var oner = function(){
...
var actions = {
  ...
  installApp: function (app, callback) {
    ...
    npmHelpers.fetchInstall(home, app, function addAppConfig (err, manifest) {
      if (err) {
        LOGGER.raw(err);
  ...
  installPlugin: function (plugin, callback) {
    ...
    npmHelpers.fetchInstall(home, plugin,
                            function addPluginConfig (err, manifest) {
      if (err) {
        LOGGER.raw(err);
