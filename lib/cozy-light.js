...
var restartWatcher = require('./restart-watcher');
...
var applicationHelpers = {
  ...
  start: function (application, db, callback) {
    ...
    if ((application.type === undefined || application.type === 'classic')
      ...
      var appModule;
      try {
        var cozyFile = pathExtra.join(configHelpers.modulePath(name),'cozy.js');
...
var actions = {
  ...
  start: function (program, callback) {
    ...
    app.use(function(q,r,n){
      if (actions.noExpressLog !== true) {
        morgand(q,r,n);
      }
    });
  ...
  exitNow: function () {
    var killProcess = function (err) {
      ...
      /*eslint-disable */
      var handles_c = process._getActiveHandles().length;
      var requests_c = process._getActiveRequests().length;
      /*eslint-enable */
      if ( handles_c+requests_c >0 ) {
        LOGGER.info('Has remaining open handles..');
        LOGGER.info('_getActiveHandles ' + handles_c);
        LOGGER.info('_getActiveRequests ' + requests_c);
      }
