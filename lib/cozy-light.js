...
module.exports = function run (home, program){
  if (has_run) {
    if ( home && configHelpers.getHomePath() !== pathExtra.resolve('' + home) ) {
      throw 'Cozy-Light already started on home ' + home;
    }
    return actions;
  }
  has_run = true;
  ...
  configHelpers.setDefaultAppsPort(18001);
  configHelpers.setMainAppPort(program.port || 19104);
  ...
  Object.keys(config.plugins || {}).forEach(function (name) {
    pluginHelpers.loadPlugin(name, program);
  });
