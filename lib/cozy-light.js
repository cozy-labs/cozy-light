      ...
      try {
        ...
        if (plugin.onExit !== undefined) {
          plugin.onExit(options, config, function (err) {
            if (err) {
              LOGGER.raw(err);
              LOGGER.info('\t' + symbols.err + '\t' + options.displayName + '');
            } else {
              LOGGER.info('\t' + symbols.ok + '\t' + options.displayName + '');
            }
            callback(err);
          });
        } else {
  ...
  start: function (application, db, callback) {
    ...
    if ((application.type === undefined || application.type === 'classic')
      ...
      try {
        ...
        if(!err.toString().match(new RegExp("Cannot find module"))) {
          LOGGER.raw(err);
        }
        try {
        ...
        try {
          if (loadedApps[name].server !== undefined ){
            loadedApps[name].server.close(function logInfo (err) {
              var appInfos = loadedApps[name];
              if (err) {
                LOGGER.raw(err);
                LOGGER.info(
                  '\t' + symbols.err + '\t' + application.displayName + '');
              } else {
                LOGGER.info(
                  '\t' + symbols.ok + '\t' + application.displayName + '');
              }
