...
module.exports = function(home, options){

  var routes = {};

  var nodeHelpers = require('./node-helpers');
  var cozyKernel = require('./cozy-kernel');
  cozyKernel.setHome(home);

  var npmHelpers = require('./npm-helpers')(cozyKernel);
  var applicationHelpers = require('./application-helpers')(cozyKernel.configHelpers, routes, defaultAppsPort);
  var pluginHelpers = require('./plugin-helpers')(cozyKernel.configHelpers);
  var mainAppHelper = require('./main-app-helper')(cozyKernel.configHelpers, routes);

  var cozyLight = {
    configHelpers: cozyKernel.configHelpers,
  ...
  cozyLight.actions = {
    ...
    restart: function (callback) {
      ...
      cozyLight.actions.stop(function(){
        configHelpers.loadConfigFile();
        loadedPlugins = {};
        loadedApps = {};
        routes = {};
        LOGGER.info('Starting all apps...');
        cozyLight.actions.start(program, function(){
     ...
     */
    exitNow: function () {
      cozyLight.actions.exit(function killProcess (err) {
    ...
    installDistro: function (distro, callback) {
      if (distro !== undefined && distro !== '') {
        try {
          ...
          distrib.installDistro(distro, cozyLight.actions, function(err) {
            if (err) {
